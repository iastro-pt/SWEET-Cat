...
# This script was used just to first add the database column in Sweet-Cat
# -----------------------------------------------------------------------
class Update:
...
if __name__ == '__main__':
    ...
    exo_nasa = Update(controversial=False, download=True, nasa=True)
    # new.update()

    # Done once
    # ADD new Database column at the end of Sweet_Cat
    names_ = ['name', 'hd', 'ra', 'dec', 'V', 'Verr', 'p', 'perr',
              'pflag', 'Teff', 'Tefferr', 'logg', 'logger',
              'n1', 'n2', 'vt', 'vterr', 'feh', 'feherr', 'M', 'Merr',
              'author', 'link', 'source', 'update', 'comment', 'n3']
    SC = pd.read_csv('WEBSITE_online.rdb', delimiter='\t', names=names_)
    SC.insert(len(SC.columns)-1, 'Database', 'EU')
    # Write to .rdb file
    SC.to_csv('website_nasa-eu.rdb', sep='\t', index=False, header=False)
    ...
    names_ = ['name', 'hd', 'ra', 'dec', 'V', 'Verr', 'p', 'perr',
              ...
              'n3']
    SC = pd.read_csv('website_nasa-eu.rdb', delimiter='\t', names=names_)

    # Check if star in NASA is in SweetCat
    # if yes, then add ',NASA' in database

    print('\n    Matching data base...')

    NewStars = []
    coordExo = coord.SkyCoord(ra=exo_nasa.exoplanet['ra'].values,
                              dec=exo_nasa.exoplanet['dec'].values,
                              unit=(u.deg, u.deg),
                              frame='icrs')
    ...
    for i, exo_name in enumerate(exo_nasa.exo_names):
        starName = exo_name
        # starName = self.exoplanet['star_name'].values[i]
        print(starName)
        # Clean star name
        ...
        if len(ind) != 0:
            ind_SC = ind[0]
            print(ind_SC)
            print('Star in NASA found in Sweet-Cat')
            if exo_nasa.nasa:
                if 'NASA' in SC.loc[ind_SC].database:
                    print('NASA is already written in sweetcat 1\n')
                else:
                    SC.at[ind_SC, 'database'] = SC.at[ind_SC, 'database'] + ',NASA'

        if len(ind) == 0:
            try:
                # it didn't find by position but it finds by name
                position = exo_nasa.sc_names.index(tmp)
                print('position', position)
                print('Star in NASA found by name in SweetCat')
                if exo_nasa.nasa:
                    if 'NASA' in SC.loc[position].database:
                        print('NASA is already written in sweetcat 2\n')
                    else:
                        SC.at[position, 'database'] = SC.at[position, 'database'] + ',NASA'

            except:
                position = -1
                # it didn't find by position and neither by name
                if (tmp not in exo_nasa.blacklist):
                    NewStars.append(starName)

    # Write to .rdb file
    SC.to_csv('website_nasa-eu_updated2.rdb', sep='\t',
